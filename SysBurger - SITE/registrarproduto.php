...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    ...
    {
        $erro = true;
        header("Location: alteraringrediente.php?msg_erro=Nome do produto já existente.");
    }
    $descricao = $_POST['descricao'];
    if(!$descricao)
    {
        $erro = true;
        header("Location: alterarproduto.php?msg_erro=Coloque a descrição do produto.");
    }
    ...
    {
        $erro = true;
        header("Location: alterarproduto.php?msg_erro=Coloque uma imagem.");
    }
    else
    {
    ...
    $sql = "INSERT INTO tb_produto (S_NM_PRODUTO, S_DESC_PRODUTO, D_VLRUNID_PRODUTO, S_IMG_PRODUTO) VALUES ('$nome', '$descricao' ,'$valor', '$img')";
    if (!$erro)
    {
...
<head>
    ...
    <title>SysBurger - Cadastro de Produto</title>
</head>
...
<body onload="loadTheme()">
    ...
    <main>
        <div class="tela_registro-produto">
            <form action="registrarproduto.php" method="post" enctype="multipart/form-data">
